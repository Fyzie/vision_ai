...
def annotate_image(image, detections, class_names, colors):
    ...
    colors = [colors[name] for name in detected_class]
    ...
    if masks is not None:
        image = mask_annotator(image, masks, colors, alpha = 0.1)
    
    image = box_annotator(image, boxes, colors)
    image = label_annotator(image, boxes, labels, colors)
