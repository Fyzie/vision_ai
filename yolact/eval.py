...
from data.config import cfg, update_config, LABEL_MAP
...
class Make_json:
    def __init__(self):
        ...
        for coco_id, real_id in LABEL_MAP.items():
            class_id = real_id - 1
...
def prep_metrics(ap_data, nms_outs, gt, gt_masks, h, w, num_crowd, image_id, make_json, cocoapi):
    ...
    with timer.env('After NMS'):
        ...
        device = pred_boxes.device  # predictions always define the target device

        pred_classes = list(pred_classes.cpu().numpy().astype(int))
        pred_confs = list(pred_confs.cpu().numpy().astype(float))
        pred_masks = pred_masks.view(-1, h * w).to(device)
    ...
    with timer.env('Prepare gt'):
        gt_boxes = torch.tensor(gt[:, :4], device=device)
        gt_boxes[:, [0, 2]] *= w
        gt_boxes[:, [1, 3]] *= h
        gt_classes = list(gt[:, 4].astype(int))
        gt_masks = torch.tensor(gt_masks, device=device).view(-1, h * w)
...
def calc_map(ap_data):
    ...
    # Store per-class AP
    per_class_ap = {'box': {cls: [] for cls in range(len(cfg.dataset.class_names))},
                    'mask': {cls: [] for cls in range(len(cfg.dataset.class_names))}}
        ...
        for iou_idx in range(len(iou_thresholds)):
            for iou_type in ('box', 'mask'):
                ...
                if not ap_obj.is_empty():
                    ...
                    per_class_ap[iou_type][_class].append(ap)
    ...
    # --- Per-class AP (averaged over IoUs) ---
    classwise_results = []
    for cls_id, cls_name in enumerate(cfg.dataset.class_names):
        # if cls_id != 0:
        box_ap = np.mean(per_class_ap['box'][cls_id]) * 100 if per_class_ap['box'][cls_id] else 0
        mask_ap = np.mean(per_class_ap['mask'][cls_id]) * 100 if per_class_ap['mask'][cls_id] else 0
        classwise_results.append([cls_name, round(box_ap, 2), round(mask_ap, 2)])

    # Print COCO-style global table
    ...
    row3.insert(0, 'mask')
    global_table = AsciiTable([row1, row2, row3])

    # Print per-class AP table
    class_table = AsciiTable([['Class', 'Box AP', 'Mask AP']] + classwise_results)

    return global_table.table, class_table.table, row2, row3
...
def evaluate(net, dataset, max_num=-1, during_training=False, cocoapi=False, traditional_nms=False):
    ...
    for i, image_idx in enumerate(dataset_indices):
        ...
        global_table, class_table, box_row, mask_row = calc_map(ap_data)
        print(global_table)
        print("\nPer-class results:")
        print(class_table)
        return global_table, class_table, box_row, mask_row
