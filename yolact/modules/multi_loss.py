...
class Multi_Loss(nn.Module):
    ...
    def forward(self, predictions, box_class, mask_gt, num_crowds):
        ...
        losses['loss_box_reg'] = self.bbox_loss(pos_box_p, pos_offsets)
        losses['loss_mask'] = self.lincomb_mask_loss(positive_bool, prior_max_index, coef_p, proto_p, mask_gt, prior_max_box)
        losses['loss_classifier'] = self.ohem_conf_loss(class_p, conf_gt, positive_bool)
        if cfg.train_semantic:
            losses['loss_semantic'] = self.semantic_segmentation_loss(predictions['segm'], mask_gt, class_gt)
