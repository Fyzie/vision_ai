...
def draw_img(results, img_origin, img_name, hide_mask=False, hide_bbox=False,
                                     hide_score=False, cutout=False,
                                     real_time = False, fps=None):
    ...
    if not hide_bbox:
        for i in reversed(range(num_detected)):
            ...
            text_str = f'{class_name}: {classes[i]:.2f}' if not hide_score else class_name
