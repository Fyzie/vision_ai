    ...
    def __call__(self, image, masks, boxes=None, labels=None):
        ...
        while True:
            ...
            for _ in range(50):
                ...
                # CORRECTION: need two args for random.uniform for left/top
                ...
                # keep the original weird logic preserved (matching upstream)
...
class Expand(object):
    ...
    def __call__(self, im, masks, boxes, labels):
        # keep consistent variable name 'im' — don't return undefined 'image'
        if not (random.randint(0, 1)):  # 50% chance to expand; rand int returns 0 or 1
        ...
        expand_image = np.zeros((int(height * ratio), int(width * ratio), depth), dtype=im.dtype)
        expand_image[:, :, :] = MEANS
        expand_image[int(top):int(top + height), int(left):int(left + width)] = im
        image = expand_image  # keep backwards-compatible name 'image' if later code expects it
